...
janus_io_info *init(janus_io *janus_io) {
	janus_io->init("");
	janus_io_info *io_info = g_malloc(sizeof(janus_io_info));
	io_info->path = "https://spark-courseware.oss-cn-shenzhen.aliyuncs.com/dev/sloth/test.whiteboard";
...
int main() {
	...
	if (!io) {
		...
		janus_io_info *io_info = init(janus_io);
