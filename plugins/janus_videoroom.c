...
/* version 1 + msg_type 1 + size 8 + chunk 1 + index 4 = 15 byte */
#define JAVUS_VIDEOROOM_DATA_PKT_HEADER_LEN 15
/* data packet version */
#define JANUS_DATA_PKT_VERSION 1
...
static struct janus_json_parameter whiteboard_parameters[] = {
	{"room", JSON_INTEGER, JANUS_JSON_PARAM_REQUIRED | JANUS_JSON_PARAM_POSITIVE},
...
typedef struct janus_xiao_data_packet_header {
	unsigned char version;
	unsigned char msg_type;
	guint64 total_size;
} janus_xiao_data_packet_header;

typedef struct janus_xiao_data_packet {
	unsigned char version;
	unsigned char msg_type;
	guint64 total_size;
	char  *xiao_data_packet_buf;
	guint64 xiao_data_packet_received;
} janus_xiao_data_packet;
...
typedef struct janus_videoroom_participant {
	...
	char  *xiao_data_packet_buf;
	guint64 xiao_data_packet_received;
	janus_xiao_data_packet_header *xiao_data_packet_header;
	janus_mutex data_send_mutex;
	janus_mutex data_recv_mutex;
...
static void janus_videoroom_relay_xiao_data_packet(gpointer user, const char *user_type, gpointer send_data);
...
static void janus_videoroom_relay_subscriber_packet(gpointer user, gpointer send_packet);
...
typedef struct janus_videoroom_data_packet {
	gchar *data;
	gint length;
	...
	if(config != NULL) {
		...
		while(cl != NULL) {
			...
			if (videoroom->whiteboard == NULL && videoroom->whiteboard_dir && videoroom->whiteboard_filename) {
				...
				videoroom->whiteboard = janus_whiteboard_create(videoroom->whiteboard_dir, videoroom->whiteboard_filename);
...
struct janus_plugin_result *janus_videoroom_handle_message(janus_plugin_session *handle, char *transaction, json_t *message, json_t *jsep) {
	...
	if(!strcasecmp(request_text, "create")) {
		...
		JANUS_LOG(LOG_INFO, "Got a other DataChannel message (%lu bytes) to forward\n", len);
		GHashTableIter iter;
		gpointer value;
		g_hash_table_iter_init(&iter, videoroom->participants);
		while (!videoroom->destroyed && g_hash_table_iter_next(&iter, NULL, &value)) {
		...
		/* Done */
		goto plugin_response;
	} else if(!strcasecmp(request_text, "whiteboard")) {
		/* publish a message to an existing videoroom */
		JANUS_LOG(LOG_VERB, "Attempt to publish a whiteboard message to an existing videoroom\n");
		JANUS_VALIDATE_JSON_OBJECT(root, whiteboard_parameters,
			error_code, error_cause, TRUE,
			JANUS_VIDEOROOM_ERROR_MISSING_ELEMENT, JANUS_VIDEOROOM_ERROR_INVALID_ELEMENT);
		if(error_code != 0)
			goto plugin_response;
		...
		if (cmd_type == KLPackageType_AddScene) {
			json_t *json_resource = json_object_get(root, "resource");
			char *resource = g_strdup(json_string_value(json_resource));
			json_t *json_page_count = json_object_get(root, "page_count");
		...
		if (cmd_type == KLPackageType_AddScene || cmd_type == KLPackageType_ModifyScene) {
			...
			for (int i = 0; i < scene_num; i ++) {
				json_t *scene = json_array_get(scenes, i);
				JANUS_VALIDATE_JSON_OBJECT(scene, scene_parameters,
					error_code, error_cause, TRUE,
					JANUS_VIDEOROOM_ERROR_MISSING_ELEMENT, JANUS_VIDEOROOM_ERROR_INVALID_ELEMENT);
				if(error_code != 0)
					goto plugin_response;
			...
			JANUS_LOG(LOG_INFO, "Got a extension message: %lu, %s\n", strlen(extension), extension);
...
static void janus_videoroom_recorder_create(janus_videoroom_participant *participant, gboolean audio, gboolean video, gboolean data) {
	...
	if(audio) {
		...
		if(participant->recording_base) {
			...
			g_snprintf(filename, 255, "videoroom-%s-%s-%"SCNu64"-audio-user",
				participant->room->room_name, participant->display, now);
			participant->arc = janus_recorder_create(participant->room->rec_dir,
				janus_videoroom_audiocodec_name(participant->acodec), filename);
			if(participant->arc == NULL) {
				JANUS_LOG(LOG_ERR, "Couldn't open an audio recording file for this publisher!\n");
	...
	if(video) {
		...
		if(participant->recording_base) {
			...
			// 	participant->room->room_id, now, participant->user_id);
			g_snprintf(filename, 255, "videoroom-%s-%s-%"SCNu64"-video-user",
				participant->room->room_name, participant->display, now);
			participant->vrc = janus_recorder_create(participant->room->rec_dir,
				janus_videoroom_videocodec_name(participant->vcodec), filename);
			if(participant->vrc == NULL) {
	...
	if(data) {
		...
		if(participant->recording_base) {
			...
			// 	participant->room->room_id, now, participant->user_id);
			g_snprintf(filename, 255, "videoroom-%s-%s-%"SCNu64"-data-user",
				participant->room->room_name, participant->display, now);
			participant->drc = janus_recorder_create(participant->room->rec_dir,
				"text", filename);
			if(participant->drc == NULL) {
	...
	if (participant->room->whiteboard == NULL) {
		...
		janus_videoroom *videoroom = participant->room;
		participant->room->whiteboard = janus_whiteboard_create(videoroom->whiteboard_dir, videoroom->whiteboard_filename);
		if(participant->room->whiteboard == NULL) {
			JANUS_LOG(LOG_ERR, "Couldn't open an data recording file for this room! Try to create next time if another audio joins.\n");
		}
	}
	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		if(session->participant_type == janus_videoroom_p_type_none) {
			...
			if(!strcasecmp(ptype_text, "publisher")) {
				...
				janus_mutex_init(&publisher->data_send_mutex);
				publisher->xiao_data_packet_buf = NULL;
				publisher->xiao_data_packet_received = 0;
				publisher->xiao_data_packet_header = NULL;
				janus_mutex_init(&publisher->data_recv_mutex);
...
static void janus_videoroom_relay_data_packet(gpointer data, gpointer user_data) {
	janus_videoroom_data_packet *packet = (janus_videoroom_data_packet *)user_data;
	janus_videoroom_listener *listener = (janus_videoroom_listener *)data;
...
static void janus_videoroom_participant_free(janus_videoroom_participant *p) {
	...
	janus_mutex_destroy(&p->data_send_mutex);
	janus_mutex_destroy(&p->data_recv_mutex);
	g_free(p);
}
...
static int janus_videoroom_wrap_datachannel_data_packet(janus_videoroom_participant *participant, char *buf, int len) {
	...
	if (chunked > 1) {
		...
		// 没有被拆分的情况
		if (participant->xiao_data_packet_buf) {
			g_free(participant->xiao_data_packet_buf);
			participant->xiao_data_packet_buf = NULL;
		}
		participant->xiao_data_packet_buf = g_malloc0(total_size + 1);
	...
	participant->xiao_data_packet_header->total_size = total_size;
	return 1;
}

/* 转发数据包。需要重新封包。确保不大于64KB一个包 | version | type | total size | chunk | index | data buf | */
static void janus_videoroom_relay_xiao_data_packet(gpointer user, const char *user_type, gpointer send_data) {
	janus_videoroom_session *session    = (janus_videoroom_session *)user;
	janus_xiao_data_packet *xiao_packet = (janus_xiao_data_packet *)send_data;

	if(!session || !session->handle || !user_type || !xiao_packet) {
		return;
	}

	char *buf = xiao_packet->xiao_data_packet_buf;
	guint64 len = xiao_packet->total_size;
	if(gateway != NULL && buf != NULL && len > 0) {
		JANUS_LOG(LOG_VERB, "Forwarding dataChannel message (%lu bytes) to %s begin.\n", len, user_type);
		unsigned char version = xiao_packet->version;
		unsigned char msg_type = xiao_packet->msg_type;
		guint64 total_size    = xiao_packet->total_size;//Byte
		...
		if (chunk_pkt == NULL) {
			JANUS_LOG(LOG_ERR, "Memory error, relay xiao_packet failed.\n");
			return;
		}
		do {
			...
			JANUS_LOG(LOG_VERB, ">Forwarding dataChannel message (%zu bytes) to %s. chunk:%d, index:%d\n", data_writing, user_type, chunked, pkt_index);
			pkt_index   += 1;
...
static void janus_videoroom_relay_participant_packet(janus_videoroom_participant *participant, janus_xiao_data_packet *xiao_packet) {
	...
	janus_videoroom_relay_xiao_data_packet(session, "publisher", xiao_packet);
	janus_mutex_unlock(&participant->data_send_mutex);
}

/* 把数据转发给订阅者 */
static void janus_videoroom_relay_subscriber_packet(gpointer user, gpointer send_packet) {
	janus_videoroom_listener *subscriber = (janus_videoroom_listener *)user;
	//janus_xiao_data_packet  *xiao_packet = (janus_xiao_data_packet *)send_packet;
	...
	janus_videoroom_relay_xiao_data_packet(session, "subscriber", send_packet);
}
